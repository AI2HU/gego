...
3. Extend `ExclusionWordDatabase` in `exclusion_words.go` (PostgreSQL-only features) or `SQLDatabase` in `sql.go` for shared SQL tables, and forward methods from `hybrid.go`.
